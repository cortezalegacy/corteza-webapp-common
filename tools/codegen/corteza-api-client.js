...
let esFormatterOptions = require('esformatter/lib/preset/default')
...
namespaces.forEach(({ path, namespace, className }) => {
  ...
  const endpoints = [].concat.apply([], spec.map(e => {
    let { get = [], post = [], path = [] } = e.parameters || {}
    const parentGet = get
    ...
    return e.apis.map(a => {
      ...
      let allvars = [...path, ...get, ...post]

      return {
        title: a.title,
        description: a.description,

        fname: _.camelCase(e.entrypoint + ' ' + a.name),
  ...
  try {
    ...
    fs.writeFileSync(`src/lib/corteza-server/rest-api-client/${namespace}.js`, gen)
  } catch (err) {
    console.error(err)
  }
})
